...
const APPS = [
  ...
  { name: 'MyCRM',       url: 'https://mycrm.com.au',               color: '#EB99C2' },
  { name: 'XPLAN',       url: 'https://iress.com',                  color: '#BBC6DA' },
]
  ...
  return (
    <div style={{ minHeight:'100vh', background:BG, backgroundImage:'radial-gradient(ellipse at 20% 50%, rgba(235,153,194,0.05) 0%, transparent 60%)', display:'flex', flexDirection:'column', fontFamily:"'DM Sans',system-ui,sans-serif" }}>
      ...
      <header style={{ display:'flex', alignItems:'center', justifyContent:'space-between', padding:'20px 48px', borderBottom:'0.5px solid rgba(187,198,218,0.15)', opacity:visible?1:0, transform:visible?'translateY(0)':'translateY(-10px)', transition:'all 0.5s ease' }}>
        ...
        <div style={{ fontSize:11, color:'rgba(187,198,218,0.45)', letterSpacing:'0.1em', textTransform:'uppercase' }}>Internal Platform</div>
      </header>

      {/* Main */}
      <main style={{ flex:1, padding:'44px 48px 32px' }}>
...
function ToolCard({ tool, index, visible, navigate }) {
  ...
  return (
    <div onClick={() => tool.active && navigate(tool.path)}
      ...
      style={{
        width:160, height:180, background:BG,
        border: hovered && tool.active ? '1px solid rgba(235,153,194,0.5)' : '1px solid rgba(187,198,218,0.15)',
        borderRadius:14, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', gap:14,
        cursor: tool.active ? 'pointer' : 'not-allowed',
        transition:'all 0.25s ease',
        transform: hovered && tool.active ? 'translateY(-4px)' : 'translateY(0)',
        boxShadow: hovered && tool.active ? '0 12px 32px rgba(235,153,194,0.12)' : 'none',
        opacity: visible ? 1 : 0,
      ...
      <div style={{ width:72, height:72, filter: tool.active ? (hovered?'brightness(1.1)':'brightness(1)') : 'brightness(0.3) saturate(0)', transition:'filter 0.25s ease' }}>
...
function AppCard({ app, index, visible }) {
  ...
  return (
    <a href={app.url} target="_blank" rel="noopener noreferrer"
      ...
      <div style={{ width:8, height:8, borderRadius:'50%', background:app.color, boxShadow: hovered ? `0 0 8px ${app.color}` : 'none', transition:'box-shadow 0.2s ease', flexShrink:0 }}/>
      <span style={{ fontSize:12, fontWeight:500, color: hovered ? '#fff' : 'rgba(187,198,218,0.7)', transition:'color 0.2s ease', whiteSpace:'nowrap', letterSpacing:'0.02em' }}>{app.name}</span>
      <svg width="10" height="10" viewBox="0 0 10 10" fill="none" style={{ opacity:hovered?0.5:0, transition:'opacity 0.2s ease', marginLeft:2 }}>
