    ...
    const loanSections = linkedLoans.map((l, li) => {
      ...
      const compRows = comps.map((c, ci) => `
        <tr style="border-bottom:0.5px solid #f1f5f9;${ci===0?'background:#fdf0f6':''}">
          ...
          <td style="padding:8px 10px;font-size:11px;color:#64748b">${c.type || '—'}</td>
          ...
          <div style="padding:0">
            ...
            <table style="width:100%;border-collapse:collapse">
              <thead><tr style="background:#f8fafc">
                ${['Lender','Type','Rate','Est. repayment'].map(h => `<th style="padding:6px 10px;font-size:9px;color:#64748b;font-weight:600;text-transform:uppercase;text-align:${['Rate','Est. repayment'].includes(h)?'right':'left'}">${h}</th>`).join('')}
              </tr></thead>
